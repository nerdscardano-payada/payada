  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{paymentLink?.title}</h1>
          {paymentLink?.description ? <p className="mt-2 text-sm text-slate-600">{paymentLink.description}</p> : null}
        </div>
        <Badge className="bg-slate-900 text-white">{isCnt ? "Token payment" : "ADA payment"}</Badge>
      </div>

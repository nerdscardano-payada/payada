  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {walletAssets?.length > 0 ? (
        <>
          ...
          <div
            ...
            onDrop={handleDrop}
            className={`rounded-2xl border-2 border-dashed p-4 text-sm transition-colors ${isDragOver ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-slate-300 bg-slate-50 text-slate-600"}`}
          >
            <div className="flex items-center gap-3">
              <Upload className="h-4 w-4" />
              <span>Drop an NFT here from the asset gallery to auto-fill the listing details.</span>
            </div>
          </div>
        </>
      ) : (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div><Label>Policy ID</Label><Input value={formData.policy_id || ""} onChange={(e) => update("policy_id", e.target.value)} placeholder="Usually auto-filled from selected wallet NFT" /></div>
        <div><Label>Asset name (hex)</Label><Input value={formData.asset_name_hex || ""} onChange={(e) => update("asset_name_hex", e.target.value)} placeholder="Usually auto-filled from selected wallet NFT" /></div>
